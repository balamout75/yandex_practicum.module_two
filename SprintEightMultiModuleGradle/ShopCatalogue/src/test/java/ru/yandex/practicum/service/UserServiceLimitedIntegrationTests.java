...
class UserServiceLimitedIntegrationTests extends BaseIntegrationTest {
    ...
    @Autowired
    private ReactiveHashOperations<String, String, PageDto> pageDtoHashOperations;
    ...
    void testChangeInCardCount(ChangeInCardCountRequest request) {
        ...
        long initialCount = catalogueService.findItem(userId, itemId).map(x -> x.count()).block();

        cartItemService.changeInCardCount(userId, itemId, actionMode).block();

        long resultCount  = catalogueService.findItem(userId, itemId).map(x -> x.count()).block();
